...
local func = Workspace:WaitForChild("Camera").Viewmodel.ChildAdded:Connect(function(tool)   
	...
	runFunction(function()
	InfiniteJump = GuiLibrary.ObjectsThatCanBeSaved.ModulesWindow.Api.CreateOptionsButton({
		Name = "InfiniteJump",
		Function = function(callback)
			if callback then
...
runFunction(function()
		...
		DragonBreath = GuiLibrary.ObjectsThatCanBeSaved.ModulesWindow.Api.CreateOptionsButton({
			Name = "DragonBreath",
			Function = function(callback)
				if callback then
					task.spawn(function()
